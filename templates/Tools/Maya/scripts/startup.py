...
def maya_setup():
    ...
    is_headless = cmds.about(batch=True)

    # create make scene validation callback
    OpenMaya.MSceneMessage.addCallback(
        OpenMaya.MSceneMessage.kAfterOpen, scene_validation
    )
    OpenMaya.MSceneMessage.addCallback(
        OpenMaya.MSceneMessage.kAfterNew, set_scene_settings
    )
    ...
    if not is_headless:
        print("================================================")
        print(
            rf"{settings.HEADER} tools load time: {round(time.time() - start_time, 2)} seconds"
        )
...
def scene_validation(*args):
    ...
    if (
        cmds.currentUnit(time=True, q=True)
        == settings.MAYA_DEFAULT_SCENE_SETTINGS["fps"]
    ):
        valid_fps = True

    if (
        cmds.currentUnit(linear=True, q=True)
        == settings.MAYA_DEFAULT_SCENE_SETTINGS["unit"]
    ):
        valid_unit = True

    if not valid_axis or not valid_fps or not valid_unit:
        result = cmds.confirmDialog(
            title=f"{settings.HEADER_SHORT} Scene Validation",
